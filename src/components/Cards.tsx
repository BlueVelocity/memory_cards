...
export default function Cards({ cardInfo, tileCount, loading, clickCard }: { cardInfo: Array<PokeData>, tileCount: number, loading: boolean, clickCard: Function }) {
  ...
  return (
    loading ? <><img src={pokeball} className="mx-auto max-w-12 mt-20 animate-bounce"></img><p>Catching P*kemon...</p></> :
      <div className="grid grid-cols-[repeat(auto-fit,minmax(7rem,1fr))] md:grid-cols-[repeat(auto-fit,minmax(12rem,1fr))] grid-auto-rows-min">
        {cardInfo ? cardInfo.map((card) => {
          return <div key={card.id} id={card.id.toString()} className="items-center p-2 m-2 flex flex-col justify-center rounded-xl hover:cursor-pointer" onClick={(e) => clickCard(e)}>
            ...
            {tileCount <= 15 && <p className="hidden md:block max-w-fit text-xs font-bold px-3 bg-red-100 rounded-lg">{card.name}</p>}
          </div>;
        }) : null}
      </div>
  );
